<template>
  <div class="user">
    <div class="head-page" v-if="isLogin">
      ...
      <div class="info">
        <div class="mobile">{{ details.mobile }}</div>
        <div class="vip">
    ...
    <div v-else class="head-page" @click="$router.push('/shouye')">
      <div class="head-img">
        <img src="@/assets/default-avatar.png" alt="" />
      </div>
      <div class="info">
    ...
    <div class="service">
      ...
      <div class="content">
        <div class="content-item">
          <van-icon name="records" @click="$router.push('/address')"/>
          <span>收货地址</span>
        </div>
        <div class="content-item">
    ...
    <div class="logout-btn" @click="quitLogin()">
     <button>退出登录</button>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async getInfo () {
      ...
      if (this.$store.getters.getInfo) {
        ...
        this.details = res.data.userInfo
      }
    },
    // 退出登录
    quitLogin () {
      this.$dialog.confirm({
        ...
        .then(() => {
          this.$store.dispatch('user/quitlogin')// 退出
        })
        .catch(() => {
          // on cancel
        })
